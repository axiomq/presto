        ...
        implements Module
{
    ...
    public void configure(Binder binder)
    {
        ...
        featureToggleStrategyMap.addBinding("AllowAll").to(AllowAllStrategy.class);
    ...
    public FeatureToggleConfiguration getFeaturesConfiguration(FeatureToggleConfig config)
    {
        if (config.getRefreshPeriod() != null) {
            return ForwardingFeaturesConfiguration.of(memoizeWithExpiration(
                    () -> {
                        log.debug("Refreshing feature toggle control from %s", config.getConfigSource());
                        return new FileBasedFeatureToggleConfiguration(config);
                    },
